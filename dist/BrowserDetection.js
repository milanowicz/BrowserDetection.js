...
module.exports = /*#__PURE__*/function () {
   ...
   * @type {boolean}
   */
  return _createClass(BrowserDetection, [{
    key: "isMobile",
    ...
    value: function checkBrowserInternetExplorer() {
      ...
      function check(RegEx) {
        if (RegEx.exec(t._userAgent) !== null) {
          ...
          try {
            HtmlTag = t._w.document.getElementsByTagName('html')[0];
      ...
      if (this._appName === 'Microsoft Internet Explorer') {
        ...
        // RegEx to find the IE version number
        RegEx = new RegExp("Trident/.*rv:([0-9][.0-9])");
        check(RegEx);

        // Check if Browser is Edge
        RegEx = new RegExp("Edge\/([0-9][.0-9])");
        if (check(RegEx)) {
          t._isEdge = true;
        }
      }
